...
OG = None
ARCH = 'vgg'
MODEL_PATH = "base_model_cifar10_32_vgg16.h5"
...
class LRFinder(Callback):
    ...
    def plot(self):
        outF = open("output_lr.txt", "a")
        for value in self.losses:
            outF.write(str(value))
            outF.write(" ")          
        outF.write("\n")
        for value in self.lrs:
          outF.write(str(value))
          outF.write(" ")
        outF.write("\n")
        outF.close()
...
def train_layer(target, rank=0):
	...
	replacement_layers.compile(tf.keras.optimizers.SGD(.1, momentum=0.9, nesterov=True), loss="categorical_crossentropy", metrics=['acc'])
	replacement_layers.fit(layer_train_gen, epochs=5, steps_per_epoch=TRAIN_SIZE / global_batch_size, callbacks=[lr_callback])
	lr_callback.plot()
	return target
